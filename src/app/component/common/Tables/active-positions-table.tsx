  ...
  const handleColor = (e:string)=>{
    ...
    if (numberGrowt > 0) {
      growt = {
       ...
       return<span className="font-semibold " style={growt}>+{e}%</span>
    }
    if(numberGrowt < 0){
      growt = {
        color: 'red'
    ...
    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
        ...
        <tbody>
          {keys.map((key:any, index) =>(
                        ...
                        <th scope="row" className="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap text">
                            <img className="w-10 h-10 rounded-full" src={key.imageId} alt="Jese image"/>
                            <div className="pl-4"> 
                                <div className="text-base font-extrabold">{key.title}</div>
                                <div className="font-semibold text-slate-400">{key.creator}</div>
                            </div>  
                        </th>
                        <td className="px-6 py-4 text-black font-semibold">
                          <span><TbCurrencyEthereum className="inline-block  text-appColor1" />{key.amount}</span>
                        </td>
                        ...
                        <td className="px-6 py-4  text-black font-semibold">
                        <span><TbCurrencyEthereum className="inline-block  text-appColor1" />{key.fees}</span>
                        </td>
                        <td className="px-6 py-4 text-black font-semibold">
