...
export class AxiosProvider {
   ...
   static getInstance(): AxiosInstance {
      ...
      if (!AxiosProvider.instance) {
         ...
         AxiosProvider.instance.interceptors.request.use((config: InternalAxiosRequestConfig) => {
            ...
            if (AxiosProvider.abortController) config.signal = AxiosProvider.abortController?.signal;
...
export const uploadFile = async (file: File | Blob, fileName = "file") => {
   try {
      ...
      const response = await Axios.post<IUploadFileResponse>("/upload/file", formData, fileUploaderConfig);
