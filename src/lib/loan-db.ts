 ...
 */
export async function getLoans(filter: LoanFilter = {}): Promise<Response<GetLoansResponse>> {
   try {
      await connectDB();
...
export async function createLoan(params: TLoan): Promise<Response<CreateLoanResponse>> {
   try {
      await connectDB();
...
export async function getLoan(id: string): Promise<Response<GetLoanResponse>> {
   try {
      ...
      if (!parsedId) return { status: 404, success: false, error: "Loan not found" };

      const loan = await Loan.findById(parsedId).lean().exec();

      if (!loan) return { status: 404, success: false, error: "Loan not found" };
...
export async function updateLoan({ id, params }: UpdateLoanParams): Promise<Response<UpdateLoanResponse>> {
   try {
      await connectDB();
      ...
      const loan = await Loan.findByIdAndUpdate(parsedId, params, { new: true }).lean().exec();
...
export async function deleteLoan(id: string): Promise<Response<DeleteLoanResponse>> {
   try {
      await connectDB();
