      ...
      <section className="space-y-16">
         ...
         {loans?.map((loan) => (
            ...
            >
               ...
               <dd>{loan.userAddress}</dd>
               <dt>Lender Address:</dt>
               <dd>{loan.investorAddress}</dd>
               ...
               <dd>{new Date(loan.loanPeriod).toLocaleDateString()}</dd>
               <dt>Loan Request Deadline:</dt>
               <dd>{new Date(loan.loanRequestPeriod).toLocaleDateString()}</dd>
               <dt>Initial Threshold:</dt>
               <dd>{loan.initialThreshold}</dd>
               <dt>Interest Rate:</dt>
               <dd>{loan.interestRate}</dd>
               <dt>Manager NFT</dt>
               <dd>{loan.nftManager}</dd>
               <dt>Manager NFT Version</dt>
               <dd>{loan.nftVersion}</dd>
            </dl>
         ))}
         <form
            action={action}
         ...
         >
            <input type="text" name="id" placeholder="ID" className="border rounded px-2 py-1 flex-1" />
            <button className="px-4 py-1 text-black rounded bg-pink-500">Delete</button>
         </form>
      </section>
   );
};
